...
local WISP_TYPE = CONFIG.WispType
local LEAGUES = toBool(CONFIG.Leagues)
local HATCHET_OF_DIVINITY = toBool(CONFIG.HatchetOfDivinity)
local MEMORY_DOWSER = toBool(CONFIG.MemoryDowser)

local CONVERSION_MODE_MAP = {
    ["Convert memories into XP, keep energy"] = 0,
    ["Convert memories and energy into XP"] = 1,
    ["Convert memories into energy"] = 2
}
local DESIRED_CONVERSION_MODE = CONVERSION_MODE_MAP[CONFIG.ConversionMode] or 1
...
local function depositMemories()
    local wispConfig = WISP_DATA[WISP_TYPE]

    while API.Read_LoopyLoop() do
        local memoryCount = Inventory:GetItemAmount(wispConfig.memory)
        local enrichedMemoryCount = 0
        if wispConfig.enriched_memory then
            enrichedMemoryCount = Inventory:GetItemAmount(wispConfig.enriched_memory)
        end

        if memoryCount == 0 and enrichedMemoryCount == 0 then
            print("All memories deposited successfully")
            return true
        end

        print(string.format("Depositing memories: %d normal, %d enriched", memoryCount, enrichedMemoryCount))

        local empoweredRift = API.GetAllObjArray1({ID.EMPOWERED_RIFT}, 50, {0})
        local normalRift = API.GetAllObjArray1({ID.RIFT}, 50, {12})

        if #empoweredRift > 0 then
            print("Using Empowered Rift to deposit")
            API.DoAction_Object1(0xc8, API.OFF_ACT_GeneralObject_route0, {ID.EMPOWERED_RIFT}, 50)
        elseif #normalRift > 0 then
            print("Using Energy Rift to deposit")
            API.DoAction_Object1(0xc8, API.OFF_ACT_GeneralObject_route0, {ID.RIFT}, 50)
        else
            print("ERROR: No rift found to deposit memories!")
            return false
        end

        if not waitForCondition(function() return API.ReadPlayerMovin2() end, 3, 100) then
            print("WARNING: Movement did not begin after interacting with rift")
        end

        local lastChangeTime = API.ScriptRuntime()
        local lastMemoryCount = memoryCount
        local lastEnrichedCount = enrichedMemoryCount

        while API.Read_LoopyLoop() do
            API.RandomSleep2(300, 150, 150)

            memoryCount = Inventory:GetItemAmount(wispConfig.memory)
            enrichedMemoryCount = 0
            if wispConfig.enriched_memory then
                enrichedMemoryCount = Inventory:GetItemAmount(wispConfig.enriched_memory)
            end
            ...
            if memoryCount ~= lastMemoryCount or enrichedMemoryCount ~= lastEnrichedCount then
                lastChangeTime = API.ScriptRuntime()
                lastMemoryCount = memoryCount
                lastEnrichedCount = enrichedMemoryCount
            end

            if (API.ScriptRuntime() - lastChangeTime) >= 6 then
                print(string.format("WARNING: Issues depositing memories. Retrying... Remaining: %d normal, %d enriched", memoryCount, enrichedMemoryCount))
                break
            end
        end
    end
...
local function handleWisp()
    ...
    if manualDeposit then
        ...
        if Inventory:IsFull() and (memoryCount > 0 or enrichedMemoryCount > 0) then
            if not depositMemories() then
                print("ERROR: Failed to deposit memories")
                return false
            end
        end
    end
...
local function changeConversionMode(targetMode)
    local modeNames = {
        [0] = "Convert memories into XP, keep energy",
        [1] = "Convert memories and energy into XP",
        [2] = "Convert memories into energy"
    }

    print("Opening rift settings to change conversion mode to: " .. modeNames[targetMode])

    local empoweredRift = API.GetAllObjArray1({ID.EMPOWERED_RIFT}, 50, {0})
    local normalRift = API.GetAllObjArray1({ID.RIFT}, 50, {12})

    if #empoweredRift > 0 then
        print("Found Empowered Rift, opening settings...")
        API.DoAction_Object1(0x29, API.OFF_ACT_GeneralObject_route1, {ID.EMPOWERED_RIFT}, 50)
    elseif #normalRift > 0 then
        print("Found Energy Rift, opening settings...")
        API.DoAction_Object1(0x29, API.OFF_ACT_GeneralObject_route1, {ID.RIFT}, 50)
    else
        print("ERROR: No rift found to change conversion settings!")
        return false
    end
    ...
    if not interfaceOpened then
        print("ERROR: Failed to open conversion mode interface")
        return false
    end

    print("Interface opened, selecting conversion mode...")

    if targetMode == 2 then
        API.DoAction_Interface(0x24, 0xffffffff, 1, 131, 10, -1, API.OFF_ACT_GeneralInterface_route)
    elseif targetMode == 0 then
        API.DoAction_Interface(0x24, 0xffffffff, 1, 131, 13, -1, API.OFF_ACT_GeneralInterface_route)
    ...
    local modeUpdated = waitForCondition(function()
        return API.GetVarbitValue(40524) == targetMode
    end, 3, 200)

    if not modeUpdated then
        print("ERROR: Unable to update conversion settings")
        return false
    end

    print("Conversion mode successfully changed to: " .. modeNames[targetMode])
    return true
end

local function validateConfiguration()
    if HATCHET_OF_DIVINITY and MEMORY_DOWSER then
        print("ERROR: Invalid Config - Hatchet and Dowser both selected.")
        return false
    end

    if HATCHET_OF_DIVINITY and not LEAGUES then
        print("ERROR: Invalid Config - Hatchet of Divinity only exists in Leagues.")
        return false
    end
    ...
    local currentConversionMode = API.GetVarbitValue(40524)
    if currentConversionMode ~= DESIRED_CONVERSION_MODE then
        local modeNames = {
            [0] = "Convert memories into XP, keep energy",
            [1] = "Convert memories and energy into XP",
            [2] = "Convert memories into energy"
        }
        print(string.format("Conversion mode mismatch! In-game: %s, Config: %s",
            modeNames[currentConversionMode] or "Unknown",
            modeNames[DESIRED_CONVERSION_MODE] or "Unknown"))
        print("Attempting to change conversion mode automatically...")
        if not changeConversionMode(DESIRED_CONVERSION_MODE) then
            return false
        end
    end

    return true, wispConfig
end
...
local function printStartupInfo(wispConfig)
...
local function mainLoop()
    ...
    local isValid, wispConfig = validateConfiguration()
    if not isValid then
        print("Script terminated.")
        return
    end

    printStartupInfo(wispConfig)
